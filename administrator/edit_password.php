...
// Get administrator email from session
$administratorEmail = $_SESSION['email'];

// Query the administrator data from the database based on email
$query = $con->prepare("SELECT * FROM administrator WHERE email=?");
$query->execute([$administratorEmail]);

// Fetch the administrator data
$administrator = $query->fetch();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    elseif (!preg_match('/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[\W_]).*$/', $password)) {
        ...
        $stmt = $con->prepare("UPDATE administrator SET password=? WHERE email=?");
        $stmt->execute([$password, $administratorEmail]);
    ...
    <div class="container">
        <div class="site-navigation">
            ...
            <ul class="js-clone-nav d-none d-lg-inline-block text-right site-menu float-left">
                <li class=""><a href="dashboard.php">الصفحة الرئيسة</a></li>
        ...
        <div class="row align-items-center">
            <div class="col-lg-6 mx-auto text-center">
                <div class="intro-wrap">
                    <h1 class="mb-0">تعديل كلمة المرور للمدير</h1>
...
<?php if ($successMsg):  ?>
    <div id="successMessage" class="d-flex justify-content-center py-3">
        <div class="alert alert-success w-25 text-center mb-5
"  role="alert">
            <?php echo $successMsg ?>
        </div>
    </div>
...
<div class="justify-content-center d-flex text-center center-div bg-white p-5 mt-5 rounded shadow">
    <form method="post"
      action="<?php
...
<div class="mb-3">
    ...
    <button type="submit" name="editProfile" class="btn py-2 px-4 btn-primary">
