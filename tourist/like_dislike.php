...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $con->prepare("SELECT * FROM `like` WHERE tourist_id = ? AND ((review_id = ? AND type = 'disliked') OR (reply_id = ? AND type = 'disliked'))");
    $stmt->execute([$tourist_id, $review_id, $reply_id]);
    $alreadyDisliked = $stmt->rowCount() > 0;

    if ($alreadyLiked || $alreadyDisliked) {
        // If the tourist has already performed the action, show an error message and redirect
        $successMsg = 'لقد أضفت بالفعل أعجاب او عدم أعجاب لهذه المراجعة';
    ...
    if ($type === 'review') {
        ...
        if ($action === 'like') {
            ...
            $successMsg = 'تم إضافة الاعجاب';
        }
        // If the action is 'disliked'
        elseif ($action === 'disliked') {
            // Insert a new record indicating that the tourist disliked the review
            $stmt = $con->prepare("INSERT INTO `like` (type, review_id, reply_id, tourist_id) VALUES (?,?,?,?)");
            $stmt->execute(['disliked', $review_id, null, $tourist_id]);
            $successMsg = 'تم إضافة عدم الإعجاب';
        }
    }
    ...
    elseif ($type === 'reply') {
        ...
        // If the action is 'disliked'
        elseif ($action === 'disliked') {
            // Insert a new record indicating that the tourist disliked the reply
            $stmt = $con->prepare("INSERT INTO `like` (type, review_id, reply_id, tourist_id) VALUES (?,?,?,?)");
            $stmt->execute(['disliked', null, $reply_id, $tourist_id]);
            $successMsg = 'تم إضافة عدم الإعجاب';
        }
    }
